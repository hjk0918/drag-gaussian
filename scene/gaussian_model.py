...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        self._generation = torch.zeros(self.get_xyz.shape[0], device=self.device, dtype=torch.int32)
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        ...
        # Set new gaussians' generation to -1
        new_generation = torch.full(new_xyz.shape[0], -1, dtype=torch.int32, device=self._generation.device)
        self._generation = torch.cat((self._generation, new_generation), dim=0)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, grad_percentile=None):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        if grad_percentile is not None:
            max_grad = torch.kthvalue(grads, int(grad_percentile * grads.shape[0]))[0]
    ...
    def record_anchor(self):
        self.anchor = (
            self._xyz.clone().detach(),
            self._features_dc.clone().detach(),
            self._features_rest.clone().detach(),
            self._scaling.clone().detach(),
            self._rotation.clone().detach(),
            self._opacity.clone().detach(),
        )
